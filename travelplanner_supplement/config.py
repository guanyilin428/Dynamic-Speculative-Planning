...
class Config:
    ...
    TARGET_NORMAL_PROMPT = defaultdict(int)
    TARGET_NORMAL_GENERATION = defaultdict(int)
    TARGET_SP_PROMPT: int = 0
    TARGET_SP_GENERATION: int = 0
    
    APPROX_SP_PROMPT: int = 0
    APPROX_SP_GENERATION: int = 0
    APPROX_NORMAL_PROMPT = defaultdict(int)
    APPROX_NORMAL_GENERATION = defaultdict(int)
    
    TOTAL_SP_TIME: float = 0.0
    TARGET_NORMAL_TIME = defaultdict(float)
    APPROX_NROMAL_TIME = defaultdict(float)
    ...
    def reset_task_metrics(self):
        ...
        self.APPROX_NROMAL_TIME.clear()
        
        self.PREDICT_K.clear()
        self.PREDICT_CORRECT = 0
        self.PREDICT_TOTAL = 0
        self.BUILD_TRAJ_TIMES = 0
        self.PENDING_BACKGROUND_TASKS.clear()
        self.HIL_INTERACTION = 0
