"""Runner for the speculative planning system."""
...
from .planner import SpeculativePlanner
from .agents import setup_assistants
from .agents import setup_multi_agent, create_agent
from .openagi_utils import load_data, create_prompt
from .predictor import DistilBERTValueFunction
from .async_online_utils import OnlineLearningExecutor
from util import Logger

config = Config()  # Create global config instance

async def _run_planner(planner, args, prompt):
    """Async helper function to run the planner."""
    return await planner.run(args, prompt)

def parse_args():
    parser = argparse.ArgumentParser(description='OpenAGI')
    parser.add_argument('--data', type=str, default='data/openagi_task_description.txt',
                      help='data directory')
    parser.add_argument('--k', type=int, default=2,
                      help='number of approximation steps to generate everytime')
    parser.add_argument('--approx_type', type=str, default='direct',
                      help='cot, direct')
    parser.add_argument('--target_type', type=str, default='react',
                      help='react, multi_agent')
    parser.add_argument('--model_type', type=str, default="gpt-4.1-mini",
                      help='gpt-4.1-mini, deepseek')
    parser.add_argument('--pred', action='store_true',
                      help='enable speculative planning with predictor')
    parser.add_argument('--no-pred', dest='pred', action='store_false',
                      help='disable speculative planning with predictor')
    parser.set_defaults(pred=True)
    
    # Online learning parameters
    parser.add_argument('--lr', type=float, default=1e-5,
                      help='online learning lr')
    parser.add_argument('--ep', type=int, default=3,
                      help='online learning epoch per train')
    parser.add_argument('--bf', type=int, default=2500,
                      help='online learning buffer size')
    parser.add_argument('--bs', type=int, default=16,
                      help='online learning batch size')
    parser.add_argument('--gma', type=float, default=1,
                      help='online learning gamma for lambda return calculation')
    parser.add_argument('--lmd', type=float, default=0.95,
                      help='online learning lambda for lambda return calculation')
    parser.add_argument('--load', dest='load', action='store_true',
                      help='load previous trajectory and model')
    parser.add_argument('--no-load', dest='load', action='store_false',
                      help='do not load previous trajectory and model')
    parser.add_argument('--tau', type=float, default=0.5,
                      help='expectile loss tau')
    parser.add_argument('--s_task', type=int, default=1,
                      help='start task id')
    parser.add_argument('--freq', type=int, default=1,
                      help='online learning training frequency')
    parser.add_argument('--offset', type=int, default=0,
                      help='biased inference offset for k')
...
def setup_agents(args):
    ...
    app_assistant, tar_assistant = setup_assistants(args.model_type)
    
    # Set up multi-agent if needed before creating agents
    if args.target_type == 'multi_agent':
        tar_assistants = setup_multi_agent(args.model_type, tar_assistant)
    else:
        tar_assistants = tar_assistant
    
    # Create agents with proper types - loggers will be set by planner
    ...
    tar_agent = create_agent(
        args.target_type,
        tar_assistants,  # This will be either single assistant or list for multi-agent
        logger=None, 
        encoding=tiktoken.get_encoding("cl100k_base"), 
        config=config
    )
    
    return app_agent, tar_agent
...
def log_task_metrics(logger, step_num, config):
    ...
    normal_app_time = sum(config.APPROX_NORMAL_TIME[i] for i in range(1, step_num+1))
...
def run_task(args, task_id, executor, encoding, planner, traj_file):
    """Run a single task with the speculative planner."""
    # Reset config metrics for new task
    config.reset_task_metrics()
    
    # Set up task-specific loggers
    logger, target_logger, approximation_logger = setup_task_loggers(args, task_id)
    
    # Update planner loggers - planner will handle passing loggers to agents
    planner.set_loggers(logger, target_logger, approximation_logger)

    # Load and set up task
    tasks = load_data(args.data)
    task_description = tasks[task_id]
    executor.set_initial_task_prompt(task_description)
    
    # Log task description through planner's loggers
    planner.log_task_description(task_description)
    
    # Create initial prompt and run planner
    prompt = create_prompt(task_description)
    steps = asyncio.run(_run_planner(planner, args, prompt))
    
    # Log results and metrics through planner
    planner.log_results(steps)
    log_task_metrics(planner.logger, len(steps), config)
    
    # Save trajectory
    if not args.pred:
        task_traj_file = traj_file.format(task_id)
        planner.logger.log(f'k = {args.k}')
    else:
        task_traj_file = traj_file
        planner.logger.log('dynamic k')
    
    executor.save_trajectory(task_traj_file, config.ENABLE_TRAIN)
...
def main():
    ...
    # Set up agents without loggers - planner will handle logger assignment
    app_agent, tar_agent = setup_agents(args)
    
    # Initialize the planner that will be reused across tasks
    ...
    for task_id in task_ids:
        ...
        run_task(args, task_id, executor, encoding, planner, traj_file)
        warmup_task += 1
